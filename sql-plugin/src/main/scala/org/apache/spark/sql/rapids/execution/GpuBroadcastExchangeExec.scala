 ...
 * `Array[InternalRow]` (Layer 1) or projected GPU-resident batch (Layer 2) on the broadcast
 * value across DPP probe sites that share the same broadcast.
 *
...
class SerializeConcatHostBuffersDeserializeBatch(
  ...
  /**
   * Layer 1 cache: memoized `Array[InternalRow]` keyed by the projection spec. Hit once per
   * unique [[ProjectedRowsKey]] per executor for the lifetime of this broadcast value, so the
